...
function updatePlayerFly(dt) {
  ...
  p.x += mx * spd * dt;
  p.fy += my * spd * dt;
  p.x = Math.max(G.camX + 14, Math.min(G.camX + WVW * 0.75, p.x));
  p.fy = Math.max(16, Math.min(WVH - 16, p.fy));
...
function onScreen(e) {
  return e.x > G.camX - 24 && e.x < G.camX + WVW + 24 && e.y > G.camY - 24 && e.y < G.camY + WVH + 24;
}
