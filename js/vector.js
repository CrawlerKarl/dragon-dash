...
const V_HAIR = {
  goku:   [[-12, -2], [-10, -10], [-6, -14], [-1, -16], [3, -13], [8, -15], [11, -8], [13, -2]],
  gokuSS: [[-8, -8], [-5, -16], [-2, -21], [2, -22], [5, -17], [8, -10], [10, -4]],
  vegeta: [[-7, -6], [-5, -15], [-2, -20], [1, -22], [4, -19], [6, -13], [8, -6]],
};
...
function vPose(pose, t) {
  ...
  switch (pose) {
    case 'run': return {
      ...
      armB: [-5 - swing * 5, -20], armF: [-5 + swing * 5, -20], lean: 0.13,
    };
    case 'jump': return { legB: [-3, -7], legF: [6, -4], armB: [-9, -26], armF: [8, -28], lean: 0.05 };
    case 'punch': return { legB: [-4, 0], legF: [6, 0], armB: [-8, -19], armF: [16, -26], lean: 0.12 };
    case 'blast': return { legB: [-4, 0], legF: [6, 0], armB: [-8, -19], armF: [15, -27], lean: 0.07 };
    case 'charge': return { legB: [-5, 0], legF: [6, 0], armB: [-9, -17], armF: [9, -17], lean: 0 };
    case 'kame': return { legB: [-5, 0], legF: [6, 0], armB: [14, -25], armF: [15, -27], lean: 0.12 };
    case 'hurt': return { legB: [-5, -4], legF: [5, -2], armB: [-11, -31], armF: [10, -32], lean: -0.12 };
    case 'fly': return { legB: [-2, 2], legF: [1, 3], armB: [2, -44], armF: [5, -46], lean: 0 };
    default: return {
      legB: [-4, 0], legF: [5, 0], armB: [-7, -17 + Math.sin(t * 2.4) * 0.5], armF: [7, -17 + Math.sin(t * 2.4) * 0.5], lean: 0,
    };
  }
}
...
function drawVChar(g, x, y, o) {
  ...
  if (armored) {
    ...
    if (o.style !== 'frieza') {
      ...
      // abdomen crest (vegeta-style)
      g.fillStyle = '#ffd230';
      g.fillRect(-2.6, -20.4, 5.2, 2.0);
    }
    ...
    // blue obi knot + undershirt collar + kanji badge
    g.fillStyle = pal.under;
    g.beginPath(); g.moveTo(-3, shY - 1.8); g.quadraticCurveTo(1, shY + 1.5, 5, shY - 1.8);
    g.quadraticCurveTo(1, shY - 0.2, -3, shY - 1.8); g.closePath(); g.fill();
    g.fillStyle = '#fff3da';
  ...
  if (o.style === 'goku' || o.ss) {
    g.fillStyle = pal.under;
    g.beginPath(); g.ellipse(0.5, hipY - 0.4, 2.2, 1.6, 0, 0, 7); g.fill();
  }
  ...
  // ---- face: shonen eyes + serious brows ----
  const ex = hx + 2.6, ey = hy + 0.2;
  const iris = o.style === 'frieza' ? '#d01818' : (o.ss ? '#0a9a70' : '#1a1a2a');
  // eye whites
  g.fillStyle = '#ffffff';
  g.strokeStyle = 'rgba(18,18,40,0.7)'; g.lineWidth = 0.8;
  g.beginPath(); g.ellipse(ex - 0.4, ey, 2.1, 2.6, -0.08, 0, 7); g.fill(); g.stroke();
  g.beginPath(); g.ellipse(ex + 4.4, ey + 0.1, 1.6, 2.3, 0.08, 0, 7); g.fill(); g.stroke();
  // iris + glint
  g.fillStyle = iris;
  g.beginPath(); g.ellipse(ex + 0.2, ey + 0.3, 1.1, 1.6, 0, 0, 7); g.fill();
  g.beginPath(); g.ellipse(ex + 4.8, ey + 0.4, 0.95, 1.45, 0, 0, 7); g.fill();
  g.fillStyle = '#ffffff';
  g.beginPath(); g.arc(ex + 0.6, ey - 0.3, 0.45, 0, 7); g.fill();
  // brows: angled down toward nose (determined)
  g.strokeStyle = o.ss ? '#c89a00' : (o.style === 'frieza' ? '#6a2a9a' : '#16161f');
  g.lineWidth = 1.6;
  const brow = o.style === 'vegeta' ? 1.4 : 0.8; // vegeta scowls harder
  g.beginPath(); g.moveTo(ex - 2.6, ey - 3.6); g.lineTo(ex + 1.6, ey - 2.8 - brow + 1); g.stroke();
  g.beginPath(); g.moveTo(ex + 6.4, ey - 3.4); g.lineTo(ex + 3.4, ey - 2.8 - brow + 1); g.stroke();
  g.lineWidth = 1;
  // nose + mouth
  g.strokeStyle = 'rgba(120,70,50,0.9)';
  g.beginPath(); g.moveTo(ex + 2.6, ey + 1.6); g.lineTo(ex + 3.2, ey + 2.2); g.stroke();
  g.beginPath();
  if (o.pose === 'hurt') { g.arc(ex + 2.2, ey + 4.4, 1.4, 0, 7); }
  else if (o.pose === 'kame' || o.pose === 'charge' || o.pose === 'punch') { g.arc(ex + 2.2, ey + 3.9, 1.9, 0.15, Math.PI - 0.15); }
  else { g.moveTo(ex + 0.6, ey + 4.2); g.quadraticCurveTo(ex + 2.6, ey + 5.2, ex + 4.6, ey + 4); }
  g.stroke();
  // cheek shade
  g.fillStyle = 'rgba(200,110,60,0.16)';
  g.beginPath(); g.ellipse(ex + 4.6, ey + 2.6, 1.8, 1, 0.3, 0, 7); g.fill();
